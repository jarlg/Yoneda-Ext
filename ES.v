...
(** For proofs related to pushouts we have to quantify over the hypotheses to the right of [:] in order to make the induction go through. The proofs have similar structure, so an abstract approach (or tactic) could be warranted in the future. *)
...
Definition rap_pushout_zig `{Univalence} {n : nat}
  : forall {B A A' : AbGroup} (f : A $-> A') (E F : ES n B A),
    es_zig E F -> es_zig (es_pushout f E) (es_pushout f F).
Proof.
  induction n as [| [|n] IH].
  1,2: by intros ? ? ? ? ? ? []. (* path induction *)
  ...
  exists phi; split.
  2: exact q.
  nrefine (transport (fun X => es_zig (es_pushout f E0) X) _ _).
...
Definition es_splice_point_abses `{Univalence} {n : nat} {C B A : AbGroup}
  (E : ES n B A) : es_zig pt (es_abses_splice E (pt : AbSES C B)).
Proof.
  destruct n.
